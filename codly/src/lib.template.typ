...
#let codly-line(
  ...
  for hl in highlights {
    ...
    let _stroke = highlight-stroke(hl.fill)
...
#let __codly-raw(
  ...
  let highlight-inset = %highlight-inset(extra)%
  show raw.line.where(label: <codly-highlighted>): codly-line.with(
    %highlight-stroke(extra)%,
    %highlight-fill(extra)%,
    %highlight-radius(extra)%,
    highlight-inset,
    __codly-inset(highlight-inset).bottom,
    %highlight-outset(extra)%,
    %highlight-clip(extra)%,
    %default-color(extra)%,
    %highlights(extra)%,
    %smart-indent(extra)%,
    %reference-by(extra)%,
    %reference-sep(extra)%,
    %reference-number-format(extra)%,
  ...
  let range = %range(extra)%
  let ranges = %ranges(extra)%
  ...
  let display-name = %display-name(extra)%
  let display-icons = %display-icon(extra)%
  let lang-outset = %lang-outset(extra)%
  let language-block = {
    let fn = %lang-format(extra)%
    if fn == none {
      (..) => none
    } else if fn == auto {
      auto
  ...
  let default-color = %default-color(extra)%
  let radius = %radius(extra)%
  let offset = %offset(extra)%
  let offset-from = %offset-from(extra)%
  if offset-from != none {
  ...
  let _stroke = %stroke(extra)%
  let padding = __codly-inset(%inset(extra)%)
  let breakable = %breakable(extra)%
  let fill = %fill(extra)%
  let zebra-color = %zebra-fill(extra)%
  let numbers-format = %number-format(extra)%
  let numbers-alignment = %number-align(extra)%
  let number-placement = %number-placement(extra)%
  ...
  let highlighted-lines = %highlighted-lines(extra)%
  let highlighted-default-color = %highlighted-default-color(extra)%
  ...
  let skip-line = %skip-line(extra)%
  let skip-number = %skip-number(extra)%
  let skip-last-empty = %skip-last-empty(extra)%
  let reference-sep = %reference-sep(extra)%
  let reference-number-format = %reference-number-format(extra)%
  let reference-by = %reference-by(extra)%
  if not reference-by in ("line", "item") {
    panic("codly: reference-by must be either 'line' or 'item'")
  }

  let annotation-format = {
    let fn = %annotation-format(extra)%
    if fn == none {
      _ => none
    } else {
      fn
  ...
  let annotations = {
    let annotations = %annotations(extra)%
  ...
  // prepare the header
  let header = %header(extra)%
  let header-repeat = %header-repeat(extra)%

  // The language block (icon + name)
  let languages = %languages(extra)%
  let language-block = if alias == none {
    ...
    let name = if type(lang) == str {
      lang
    } else if display-name and "name" in lang {
      lang.name
    } else {
      []
    }
    ...
    if language-block == auto {
      let radius = %radius(extra)%
      let padding =__codly-inset(%lang-inset(extra)%)
      let lang-stroke = %lang-stroke(extra)%
      let lang-fill = %lang-fill(extra)%
      ...
      let lang-stroke = if type(lang-stroke) == function {
  ...
  let header = if header != none {
    let header-args = %header-cell-args(extra)%
    let transform = %header-transform(extra)%
  ...
  let smart-skip = %smart-skip(extra)%
  ...
  for line in it.lines {
    ...
    if had-first or (
      display-name != true and display-icons != true
    ) {
  ...
  // prepare the footer
  let footer = %footer(extra)%
  let footer-repeat = %footer-repeat(extra)%
  let footer = if footer != none {
    let footer-args = %footer-cell-args(extra)%
    let transform = %footer-transform(extra)%
  ...
  let block_content = block(
    ...
    width: 100%,
    radius: radius,
    stroke: if numbers-outside { none } else { _stroke },
    {
      if is-complex-fill {
        ...
        grid(
          ...
          stroke: (x,y) => 
            if numbers-outside {
              ...
              (
                left: if x == 1 { _stroke } else { none },
                right: if x == idx_end { _stroke } else { none },
                top: if x != 0 and y == 0 { _stroke } else { none },
                bottom: if x != 0 and y == it.lines.len() - 1 { _stroke } else { none },
              )
            } else {
              none
            },

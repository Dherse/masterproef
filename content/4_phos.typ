...
#figurex(
    title: [ The execution model of the @phos programming language ],
    caption: [
...
#{
    set text(size: 12pt, fill: rgb(30, 100, 200))
    smallcaps[*Synthesisable execution model*]
}
...
#{
    set text(size: 12pt, fill: rgb(30, 100, 200))
    smallcaps[*Synthesisable blocks*]
}
...
The design of the @phos compiler is inspired in parts by _Clang_'s, _Rust_'s, and _Java_'s compilers #cite("clang_internals", "rust_compiler", "openjdk_hotspot"). As previously mentioned, the compilation of a @phos program into a circuit design that can be programmed onto a photonic processor is a three-step process: compilation, evaluation, and synthesis. The compiler, as it is referred to in this section, performs the compilation step. Therefore, as previously mentioned in @sec_exec_model, it takes the user's code as an input and produces bytecode for the #gloss("vm", long: true). The compiler is written in _Rust_ and is split into several components, each with a specific purpose. As will be discussed in subsequent sections, the @phos compiler is composed of a _lexer_, a _parser_, an _#gloss("ast", long: true)_, a _desugaring_ step, a _high-level intermediary representation_, a _medium-level intermediary representation_, and a _bytecode_ generator. The multiple stages of the compiler are illustrated in @fig_compiler_arch.
...
#figurex(
    title: [ Hierarchy of grammar that can be used to describe a language. ],
    caption: [

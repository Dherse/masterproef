...
However, there are elements that are generally going to be common to all of those functional requirements. And can be seen as the functional requirements for intent translation. These requirements are can be seen in @tab_functional_requirements and are discussed in the following sections.
...
#figurex(caption: [ Functional requirements for intent translation ], kind: table)[
    #tablex(
        ...
        align(left)[
            In order to be able to make use of tunability and reconfigurability, the user must be able to programmatically communicate with their programmed device. This is done through the user of a #gloss("hal", long: true), that handles communication with the device.
        ],
        [ @sec_programmability ],
...
From the physical properties and features of a photonic processor, as discussed in @sec_programmable_photonics, one can derive a set of intrinsic operations that must be supported by the processor. These operation in themselves are not required to be on the chip, but the support packages of the chip must be able to understand them and produce errors when they are not implemented. A full list with a description can be found in @tab_intrinsic_operations. In this section, the intrinsic operators will be discussed in more detail.
...
These non-synthesizable constraints, can be couple with synthesizable constraints to create a more realistic, yet very inexpensive simulation. As will be discussed in @sec_intent_simulation, simulating circuits using constraints is extremely fast. And due to the integration of constraints within the language, as will be discussed in @sec_phos_constraints, it makes them an inherent part of the user's design. Meaning that accurate, yet fast, simulations are available to the user at all times.

#{
    set text(size: 12pt, fill: rgb(30, 100, 200))
    smallcaps[*Constraints for optimization*]
}
As previously mentioned, constraints can be used as indicators for stages within the synthesis process. Therefore, it stands to reason that constraints can be used to optimize the design. The compiler can use constraints to remove unnecessary components, or to optimize the placement of said components. A simple example, a signal going through a filter might have a constraint on the wavelength, and the filter might have a constraint on the wavelength. If the compiler can prove that the filter is not needed, it can remove the filter altogether. Alternatively, if it detects that after the filter there would be so signal left, it can remove the filter and all dependent components, simplifying the design. This is just one example of how constraints can be used to optimize the design.

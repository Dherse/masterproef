...
The solver is the tool that the compiler uses to summarize and check constraints. It is used by the compiler for these two operations, where it will summarize the constraints on each signal such that it can be easily simulated, and it will verify that constraints are compatible. Additionally, in cases where the constraints depend on tunable values -- i.e values that can be changed at runtime -- the solver can use a prover and the constraints on the tunable value to determine whether the constraints are compatible. This is done by using a prover such as _Z3_ @z3. However, this is a very computationally expensive process and must therefore only be performed when necessary. This is why the compiler will only use the prover when the constraints depend on tunable values.

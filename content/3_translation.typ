...
Two of the key applications of photonic processor are telecommunication and processing @rf signals. Therefore, it stands to reason that modulators and detectors are key components that are expected to be present in most photonic processors. Additionally, based on the device, there may be an optimal type of modulator for either type -- phase modulation or amplitude modulation -- and the compiler may choose an appropriate implementation of the modulator. Additionally, the same is true for detectors, although they would generally only be used for amplitude demodulation, with phase coherent demodulation being the responsibility of the user.
...
Constraints can be of many types; the likely most common ones will be constraints on power, gain, wavelength, delay, and phase. The reason why delay and phase are different constraints is because they most often will have different semantics, where phase refers to the phase of the light within the waveguide, and the delay will mostly impact the delay of the modulated information on the signal. Since light operates at frequencies much higher than the @rf range, one can consider the phase of the light to be mostly decoupled from the signal's phase. These constraints can be used to verify the validity of the code and to inform the compiler how to optimise and generate the design. For example, the user might have a high-power signal coming onto the chip that gets split. The place-and-route system can either place the splitter close to the input, increasing mesh usage but reducing non-linearities, or closer to the components using this light, increasing mesh usage but decreasing non-linearities. One can use the input power constraint to make a decision since at high-power, there will be increased non-linearities and losses within the waveguide. Therefore, the place-and-route can use this information to decide on where to place the splitter. This is just one example of how constraints inform the compilation system and can be used to optimise the design.
...
#figurex(
    ...
    caption: [
        Example of reconfigurability on a fictitious device. Each state (a) and (b) represent a different filter. The second (b) filter has a longer ring and, therefore, a higher @fsr than the first one (a). Squares of different colours represent photonic gates in different states: blue represents through gates, green represents cross gates, and yellow represents partial gates. The grey triangles represent optical ports.
    ],
)[
    #table(
        columns: 2,
...
There are several types of simulations that may be useful for the user: the user might want to visualize the generated circuit mesh superimposed onto a schematic representation of the device, to verify that no critical components were removed through constraints, to see the usage at a glance, or to visualize whether the place-and-route performed adequately. This visualisation is already presented in the existing library and exists in @eda tools for photonics. Therefore, such visualisation facilities must be offered to the user. Especially due to the fairly early stage of research, the ability to communicate results visually is critical to the user's understanding of the results. Another kind of visualisation the user will want is the results of the simulation results. Therefore, the ecosystem must provide easy visualisation of results.

#{
    set text(size: 12pt, fill: rgb(30, 100, 200))
    smallcaps[*Applying @dry*]
}
As is the case of the simulation ecosystem, one can reuse existing tools and libraries for visualisation that are already on the market. This is an application of the #gloss("dry", long: true) principles, where one can reuse existing tools and libraries rather than rewriting them from scratch. This also allows the user to benefit from the large ecosystem of visualisation tools that already exist, and to use the tools they are most familiar with, or that gives the best results for their application. Examples of such visualisations can be seen in @fig_reconfigurability that shows the mesh and the state of each gate, and a simulation result in @fig_simulation_result_example which shows the results of a time-domain simulation using the aforementioned constraint solver.

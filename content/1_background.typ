...
Therefore, looking at @pic_hierarchy, one can build four large categories of @pic[s] based on their programmability. The first ones #link(<pic_hierarchy>)[(a)] are not programmable at all; they require no tunable elements and are therefore the simplest. The second category #link(<pic_hierarchy>)[(b)] contains circuits that have tunable elements but fixed function; the tunable element could be a tunable coupler, phase shifter, etc. and allows the designer to tweak the properties of their circuit during use for purposes such as calibration, temperature compensation, or more generally, altering the behaviour of the circuit. The third kind of @pic is the feedforward architecture #link(<pic_hierarchy>)[(c)], which means that the light is expected to travel in a specific direction; it is composed of gates, generally containing tunable couplers and phase shifters. Additionally, external devices such as high-speed modulators, amplifiers and other elements can be added. Finally, the most generic kind of programmable @pic is the recirculating mesh #link(<pic_hierarchy>)[(d)], which, while also composed of tunable couplers and phase shifters, allows the light to travel in either direction, allowing for more general circuits to be built as explored in @sec_meshes.

#figurex(
    kind: image,
    title: [ A hierarchy of programmable #gloss("pic", long: true, suffix: "s"). ],
    caption: [
        A hierarchy of programmable #gloss("pic", long: true, suffix: "s"), starting at the non-programmable single function @pic (a), moving then to the tunable @pic (b), the feedforward architecture (c) and finally to the photonic processor (d).
    ],
)[
    #table(
        columns: (auto, auto),
...
Detectors are used to turn optical signals into electrical signals. In photonic processors, there are commonly two kinds, the first being low-speed detectors used to measure optical power at several points inside of the processor and high-speed detectors that are used to demodulate high-speed signals. There are generally built from photodiodes, however, other building techniques exist. For the purposes of this thesis, only the overall functionality needs to be understood.

...
Each of these ecosystems comes with a particular set of tools in addition to the aforementioned core components. Some of these languages come with tooling directly built by the maintainers of the languages, while others leave the development of these tools to the community. However, it should be noted that, in general, tools maintained by the language maintainers directly tend to have a higher quality and broader usage than community-maintained tools.
...
#info-box(kind: "definition", footer: [Adapted from @nonoma_formatter])[
    A *linter* is a program that looks for common errors, good practices, and stylistic issues in code. It is used in conjunction with a formatter to enforce a consistent style across a codebase. They also help mitigate the risk of common errors and bugs that might occur in code.
]
...
Interestingly, as in the case of _Clippy_, some rules can also be used to suggest better, more readable ways of writing code, colloquially called good practices. For example, _Clippy_ has a rule that suggests lowering cognitive load using the rule `clippy::cognitive_complexity` @clippy_rules. This rules suggests that functions that are too complex as defined in the literature @cognitive_load should be either reworked or split into smaller, more readable code units.
...
#info-box(kind: "definition", footer: [ Adapted from #cite("unit-test", "simulation")])[
    *Simulation* is the process of running a program that simulates the behaviour of a physical device. It is used to test that @hdl[s] produce the correct state for a given input and starting state, while also checking that the program does so in the correct timing, power consumption limits, etc.
]
...
- _Python_ does not need a `main` function to be defined, as the file itself is the program's entry point.
...
#info-box(kind: "info")[
    Most @hdl languages come with pre-built implementations of adders. Usually, the compiler or synthesis tool chooses the best implementation of the adder based on constraints defined by the user. These constraints can relate to the area, power consumption or timing requirements.
]
...
#figurex(
    ...
    #tablex(
        ...
        colspanx(4)[#align(left)[
            C is a fully featured low-level language, it is performant and has a simple syntax. However, it lacks some of the more modern ecosystem components and is error-prone. Because of this, it is unsuitable for @api design since it would require the user to be familiar with memory management. It lacks many of the semantics of hardware description, making it unsuitable as a root language. However, its large array of language-implementation libraries makes it a good candidate for the implementation of a new language.
        ]],
        ...
        [],
        colspanx(4)[#align(left)[
            Rust is a modern low-level language; it is very performant, has excellent first-party tooling, is quickly growing in popularity, and is memory safe. However, it has difficult syntax and semantics that is unwelcoming for non-developers, which makes it unsuitable for either @api design or as a root language. However, its large array of language-implementation libraries, coupled with its memory and thread safety, makes it an excellent candidate for the implementation of a new language.
        ]],

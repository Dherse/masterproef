  ...
  clip: true,
{
  ...
  let node(in-index, n-index) = {
      ...
      place(dx: x - node-r, dy: y - node-r, circle(
        radius: node-r,
        stroke: node-s,
        fill: atomic-tangerine
      ))
  }
  ...
  let arrow(in-index-a, n-index-a, in-index-b, n-index-b) = {
      ...
      let p0 = (x1, y1)
      let p1 = (x1 - 0.35 * node-r * calc.cos(θab + 80deg), y1 - 0.35 * node-r * calc.sin(θab + 80deg))
      let p2 = (x1 - 0.35 * node-r * calc.cos(θab - 80deg), y1 - 0.35 * node-r * calc.sin(θab - 80deg))
...
#let f(w) = block(radius: 0.02 * w, height: w, width: w, {
  let place = place.with(top + left)
  let r = 0.025 * w
  let s = 0.1 * r
  ...
  let edge(a, b) = {
    ...
    let p0 = (x1, y1)
    let p1 = (x1 - 0.35 * r * calc.cos(θab + 80deg), y1 - 0.35 * r * calc.sin(θab + 80deg))
    let p2 = (x1 - 0.35 * r * calc.cos(θab - 80deg), y1 - 0.35 * r * calc.sin(θab - 80deg))

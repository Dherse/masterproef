...
#uquestion(footer: [ This will be answered in @feedforward_approx. ])[
...
There are many construction techniques for building @2x2[s], each with its own advantages and disadvantages. The most common ones are the Mach-Zehnder interferometers with two phase shifters. However, other techniques involve using #gloss("mems") or liquid crystals @bogaerts_programmable_2020-1 @capmany_programmable_2016 @perez_programmable_2019.

==== Detectors <sec_detectors>

Detectors are used to turn optical signals into electrical signals. In photonic processors, there are commonly two kinds: low-speed detectors used to measure optical power at several points inside the processor and high-speed detectors used to demodulate high-speed signals.

==== Modulators <sec_modulators>

Contrary to detectors, modulators turn electrical signals into optical signals. They do not do this by producing the optical signal but by modulating the phase or the amplitude of an existing optical signal.

=== Meshes <sec_meshes>

Four main kinds of meshes can be built for programmable photonics, shown in @fig_meshes: the feedforward mesh #link(<fig_meshes>)[(a)] and three kinds of recirculating mesh: hexagonal #link(<fig_meshes>)[(b)], rectangular #link(<fig_meshes>)[(c)], and triangular meshes #link(<fig_meshes>)[(d)]. It is also possible to create meshes not made of a single kind of cell, but these will not be discussed in this thesis. This section will discuss the major differences between the feedforward and the recirculating architectures. In the case of this thesis, hexagonal meshes are the primary focus. This is because they are the most capable kind of meshes @bogaerts_programmable_2020-1.
...
All of the architectures rely on the same components #cite(<bogaerts_programmable_2020-1>), those being #gloss("2x2", suffix: "s", long: true), optical phase shifters and optical waveguides. These elements are combined in all-optical gates, which can be configured to achieve the user's intent. Additionally, to provide more functionality, the meshed gates can be connected to other devices, such as high-speed modulators, amplifiers, and detectors @bogaerts_programmable_2020-1 @capmany_programmable_2016 @perez_programmable_2019.

The primary difference between the feedforward architecture and the recirculating architecture is the ability of the designer to make light travel both ways in one waveguide. As is known #cite(<ghatak_introduction_1998>), light can travel in two directions in a waveguide with little to no interactions. This means that, without additional waveguides or hardware complexity, a photonic circuit can support two guiding modes, one in each direction. This property can be used for more efficient routing and creating more complex and varied structures #cite(<bogaerts_programmable_2020-1>).

As it has been shown in #cite(<perez_programmable_2019>), recirculating meshes can create more advanced structures, such as @iir elements, whereas feedforward architectures are limited to @fir components. This is due to the inability of the feedforward architecture to express feedback loops, limiting them to @fir components, whereas recirculating meshes allow the creation of feedback loops and @iir components. Indeed, in a feedforward mesh, the typical structure being built is the Mach-Zehnder interferometer. In contrast, in a recirculating mesh, one may build structures such as ring resonators, which are inherently @iir components. Recirculating meshes can also express structures such as @mzi and can represent both @iir and @fir components.

#uconclusion[
    The recirculating mesh is more capable as it allows feedback loops and @iir components, whereas the feedforward mesh is limited to @fir components. Additionally, recirculating meshes allow light to travel in both directions in a single waveguide, allowing for more efficient routing and complex structures.
]
...
==== Telecommunications
The telecommunications industry is one of the largest existing users of photonic technologies, most notably optical fibers. Therefore, it should come as no surprise that this is one of the applicative areas of particular interest for programmable photonics. They can be used by service providers close to their customers for multiplexing, transceivers, and resource allocations, such as in fiber-to-the-home deployments @bogaerts_programmable_2020-1.
...
#udefinition(footer: [ Adapted from @netlist. ])[
...
Netlists are common abstractions in electronics to represent a list of components and their connections. However, in the case of photonics, the definition is altered and made more limited: in electronics, a net can be connected to many components. However, in the case of a photonic circuit, the port of a component is only ever connected to the port of another component, never more. This is because, in this thesis, splitters are considered components in and of themselves; therefore, splitting the signal is equivalent to adding a component to the circuit.
...
#uconclusion[
    It has been shown that, given a sufficient level of abstraction, any bidirectional photonic circuit can be represented by an equivalent, higher-level, feedforward circuit. This result is crucial for formulating the requirements for the programming interface of such a photonic processor.
]

== Non-idealities <fppga-difficulties>

Like all other photonic components, photonic processors are impacted by non-idealities, temperature dependence, and manufacturing variabilities. These impact the well-functioning of the circuit programmed within the processor. Therefore, one must consider this impact when designing the circuit, or programming the design, in the case of a photonic processor. However, photonic processors should mitigate these variations as much as possible, as they are high-level design platforms, to allow for a more efficient design process. This section will discuss these non-idealities, and solutions to automatically mitigate their impact will be proposed.

==== Temperature dependence
Most photonic components exhibit a temperature dependence, which can be exploited to build structures such as phase shifters. However, undesired temperature changes can impact the circuit in unexpected ways. One traditional way to mitigate temperature changes is to maintain the device at a constant temperature using external means, such as Pelletier elements. However, this limits the potential use cases of the device, as such means tend to be bulky and power-hungry. 

==== Wavelength dependence
In addition to the temperature dependence mentioned above, photonic components also exhibit a strong wavelength dependence. Part of this dependence is desired, such as in the case of wavelength filters. Nevertheless, in all other cases, the user expects that the device behaves with a flat frequency response. Therefore, photonic processors must also provide mitigations for this dependence.

==== Manufacturing variability
Variabilities in the manufacturing process of the @pic cause the third kind of non-idealities; they can introduce all kinds of non-idealities, such as higher losses, reflections, and more. The imperfection of the manufacturing process causes these variabilities; therefore, the resulting devices are not identical. This means that the user's design will work differently from one chip to the next. However, one of the goals of photonic processors is to act as a high-level design platform, which means that the user should not have to worry about these variabilities. 

== Initial design requirements <initial_requirements>

This section will discuss the basic design requirements for a programming interface to photonic processors. These requirements form the base of the design of the programming interface and are, therefore, crucial to the design of the interface. More requirements and details for each will be provided in @sec_intent.

==== High-level of abstraction & modularity
Ideally, the programming interface of photonic processors should be high-level enough that it is entirely abstracted from the underlying hardware, making it easier to design circuits. Furthermore, the interface should be modular, allowing users to build complex circuits from smaller, simpler building blocks. This is a crucial requirement, as it allows the user to build complex circuits from incrementally more complex building blocks, allowing them to build more advanced circuits without understanding the underlying hardware.

==== Platform independence
The code should work across devices with little to no adjustment. This would avoid the fracturing of the ecosystem that can be observed in @fpga[s], as well as reduce the burden of the user to port their code to different devices.

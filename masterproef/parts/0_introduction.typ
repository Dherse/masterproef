...
simply changing the configuration of the mesh #cite(<bogaerts_programmable_2020>).
...
photonic @fpga[s] #cite(<bogaerts_programmable_2020-1>) or photonic processors.
...
implementation #cite(<geiger_vlsi_1990>). One may draw parallels between the
...
on the "what" of the program #cite(<noauthor_imperative_2020>).
...
functionality and behavioural expectations of their design (declarative) #cite(<noauthor_imperative_2020>).
...
abstraction, close to the component-level #cite(<bogaerts_silicon_2018>). This
...
has led to efforts to simulate them using @spice #cite(<ye_spice-compatible_2022>).
...
cost and the time to market for the product #cite(<bogaerts_programmable_2020>).
...
programmable @pic[s] #cite(<bogaerts_programmable_2020>).
...
bandwidth and high-speed capabilities #cite(<marpaung_integrated_2019>).
...
simulation #cite(<ye_spice-compatible_2022>). However, @spice tools are often
lacking, especially regarding digital co-simulation, making the process
difficult #cite(<osti_1488489>), relying instead on ill-suited alternatives such
as @verilog-a.

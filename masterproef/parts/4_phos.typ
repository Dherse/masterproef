...
#ufigure(
  ...
  caption: [
    The execution model of the @phos programming language, showing the three distinct stages. Responsibilities use the same color code as @fig_responsibilities, showing the ecosystem components in orange, the user's code in green, the platform-specific code in blue, and the third-party code in purple.
  ],
  image(
...
_Hindley-Milner_ algorithm, which is a de-facto standard in modern programming languages @milner_theory_1978 @rust_compiler. Additionally, due to the simple type system employed by @phos, it should be generally easier for the compiler to infer the types of values as conflicts are less likely to occur.
...
#ufigure(
  caption: [
    Automatic optical signal splitting schemes, showing the two possible automatic splitting scheme, using either a cascade architecture (a), or a parallel architecture (b). It leads to different power ratios for all of the downstream elements.
  ],
...
#udefinition(footer: [ Adapted from @algebraic_data_type ])[
...
#udefinition(footer: [ Adapted from @aggregate_type ])[
...
Variables in @phos are declared with the `let` keyword, followed by a pattern for the variable name, followed optionally by the type of the variable, and then followed by the assignment to the value. In @phos, variables cannot be uninitialised, and must be assigned a value when they are declared, with the notable exception of signals in unconstrained contexts, see @sec_unconstrained, which have special semantics. Variables are immutable, this makes the work of the prover for reconfigurability easier, if the user whishes to update a value, they can simply recreate it. This also makes the language simpler, as the user does not need to worry about the value of a variable changing. The general form of variable declaration can be seen in @lst_ex_variable_declaration.
...
#udefinition(footer: [ Adapted from @osullivan_real_2010. ])[
...
#block(breakable: false)[
  #ufigure(
    caption: [ Example in @phos of a closure. ]
  )[
    ```phos
...
#udefinition(footer: [ Adapted from @peyton_jones_how_2004 ])[
...
#udefinition(footer: [ Adapted from @cs390. ])[
...
#figure(
  kind: raw,
  caption: [ Example in @phos of a loop. ],
)[
  ```phos
  for i in 0..5 {
      print(i);
...
Additionally, unconstrained blocks allow the user to create their signals without needing to use a source intrinsic. This semantic is useful for creating recirculating elements in the photonic processor, as it allows the user to create temporary variables containing signals.
...
Therefore, the parts of the code that perform the conversion between high-level and low-level values must be synthesisable. Of course, the photonic processor being an analog processor, it is not possible to perform this synthesis on the actual mesh. However, as mentioned in @sec_programmability, one of the compilation artefacts is the user @hal, which is generated for the user based on their design. It would therefore be possible to collect these operations and package them into the @hal, such that when the user programmatically tunes their design, the conversion is done inside of the user @hal and sent to the processor's controller as a low-level value. The name is based on the fact that @phos uses a stack-based virtual machine and that the operations are collected into the user @hal and converted into _Rust_ (the implementation language of the @hal) automatically by the compiler.
...
Expressions are a subset of statements that operate on one or more values and may produce an output value. A complete list of @phos expressions is available in @tab_exprs.
#ufigure(
  kind: table,
...
Finally, the standard library can serve as a series of examples for new users. A photonic engineer that is knowledgeable in photonic circuit design would benefit from the standard library as a source of high-quality examples onto which they may base themselves. Similarly, a software engineer knowledgeable with software development but not photonic circuit design, would benefit from the standard library as a source of high-quality examples of basic building blocks of photonic circuits. The standard library should be written in a way that is easy to understand and that is well-documented, such that it can serve as a learning resource for new users.
...
#ufigure(
  ...
  caption: [
    Compiler architecture of the @phos programming language, showing that the user code flows into the different stages of the compiler. All of these stages produce the bytecode that can be evaluated.

    This figure uses the same colour scheme as @fig_responsibilities, showing the ecosystem components in orange, the user's code in green, and the platform-specific code in blue.
  ],
  image(
...
#udefinition(footer: [ Adapted from @nystrom_crafting_2021. ])[
...
Additionally, and not shown in @lst_lexing_ex, the @phos lexer preserves the span where the token is found in the source code. This is used to generate more meaningful errors by indicating the location of the error in the source code. This is done by associating the said span with each token produced by the lexer.
...
Before parsing the language, one must first describe the grammar of the language. There exist many families of grammar, as seen in @fig_parser_hierarchy. The more complex the grammar is, the more complex of a parser it will require. It is important to note that @fig_parser_hierarchy describes grammar, not languages. Languages can be expressed using multiple grammars, and some grammars for a given language can be simpler @cs143. Every grammar can be expressed with a higher-level grammar, but the reverse is not valid.
...
#ufigure(
  ...
  caption: [
    Hierarchy of grammar that can be used to describe a language. The grammars are ordered from the most powerful to the least powerful. The most powerful grammars are able to describe any unambiguous language, whereas the least powerful grammars are only able to describe a subset of the languages @cs143. As @phos does not use ambiguous grammar and they are very difficult to describe and parse, they are not discussed further.
  ],
  image(
...
#uinfo[
  From this point in the document, the language is not yet implemented and therefore does not exist. Therefore, the following steps are not implemented and only describe what will be done when the language is fully implemented.
]

#udefinition(footer: [ Adapted from @nystrom_crafting_2021. ])[
  *Syntactic sugar* is the part of the language's syntax that is intended to make things easier to read, express, or understand.
]
...
#ufigure(
  kind: raw,
  caption: [
      An example used to show the desugaring, @ast expansion, and @ast validation in @phos.
  ],
)[
  ```phos
...
#unote[
  The processes being described in this section are done at the @ast level. However, for clarity, the source code is shown instead of the @ast for each step in this compilation stage.
]
...
==== Type inference
After lowering the @ast into the @hir, the compiler will try to infer all values' types based on existing annotation and some rules. When the compiler has inferred the type of a value, it will explicitly annotate that value with its type, that way, each value has its type known at each point in the code, such that further checks can be performed. Continuing with @lst_desug_ex, one can see what the resulting, fully annotated code would look like in @lst_type_inf_ex. It shows what the code would look like if this were valid syntax after @ast lowering and explicit type annotation.

#ufigure(
  kind: raw,
  outline: [ Example of name stripping and type inference in @phos ],
  caption: [
    Example of name stripping and type inference in @phos, showing the process of name stripping and type inference, using the example from @lst_desug_ex. All nodes are annotated with their type, and all variables, arguments, function names, etc. have been renamed with an ID shown here as `$n`, where `n` is a number.
...
#ufigure(
  caption: [
    The code example in @phos and code-equivalent representation of its @mir expanded version. Shows a function computing whether a number is prime before (a) and after expansion (b). The code in (b) is not valid @phos and is purely for demonstration purposes.
  ],
  outline: [ Code example in @phos and code-equivalent representation of its @mir expanded version. ],
  kind: raw,
  table(
    columns: 2,
    stroke: none,
...
#ufigure(
  caption: [
    The code example in @phos shows the original code (a), the code after constant inlining (b), and the code after constant evaluation (c).
...
#ufigure(
  caption: [
    The code example in @phos shows the original code (a) and the code after pipe desugaring (b).
...
#udefinition(footer: [ Adapted from @java_se_specs. ])[
...
#ufigure(
  ...
  caption: [
    Instruction set of the @phos #gloss("vm", long: true). Showing the instruction and its static arguments (i.e. arguments that are produced during compilation) and the operations that each instruction does on the stack.
  ],
  kind: table,
  tablex(
    ...
    ```,
    align(horizon)[
      Calls the function with the given ID, the arguments are obtained from the function definition and then popped from the stack, and the result of the function call is pushed onto the stack.
    ],
    ...
    ```,
    align(horizon)[
      Calls the method with the given ID on the given type, the arguments are obtained from the function definition and then popped from the stack, and the result of the function call is pushed onto the stack.
    ],
    ...
    ```,
    align(horizon)[
      Returns from the current function, popping all of the remaining values from the stack and returning them as a tuple. If the stack is empty, it returns an empty tuple, which is equivalent to the `none` value.
    ],
    ...
    ```,
    align(horizon)[
      Loads the value with the given local variable ID onto the stack. If it is a tuple, it expands the tuple into `len` values on the stack and pushes the tuple length onto the stack. The first `n` local variables are reserved for the arguments of the function, where `n` is the number of arguments of the function.
    ],
    ...
    ```,
    align(horizon)[
      Stores the top `len` values of the stack into the local variable with the given ID. If `len` is more than one, then store them as a tuple. The first `n` local variables are reserved for the arguments of the function, where `n` is the number of arguments of the function.
    ],
...
#uconclusion[
  The @phos compiler translates source code into a bytecode format used by the @vm for evaluation. It first turns the code into a computer-understandable representation called the @ast. Then the @ast goes through three transformation stages. Called the @hir, the @mir, and finally, the bytecode.
]
...
==== Licensing
Finally, existing @vm[s] may be subject to licenses. While intellectual property has not been discussed in this document, it is essential to be aware of issues that can arise when using other people's code or, more generally, intellectual property. Therefore, if the @vm is written from scratch, it can be licensed in a way that is compatible with the @phos license, whichever that may be.

=== Stack-based architecture <sec_stack_based>

#udefinition(footer: [ Adapted from @cormen_introduction_2009. ])[
...
#block(breakable: false)[
  #table(
    ...
    #ufigure(
      caption: [
        The signal flow diagram of @lst_tree_ex shows the splitting of the input signal, followed by the filtering of both signals, and finally, the gain of one of the signals. The green boxes represent the intrinsic operations, the blue boxes represent the constraints, and the arrows represent the flow of the signal.
      ],
...
#udefinition(footer: [ Adapted from @jones_partial_1993 ])[
  *Partial evaluation* is a technique for specialising a program with respect to some of its arguments. The result is a new program that only requires the remaining arguments to run. The new program is generally smaller.
]
...
Among the ongoing work that has been happening, including at the @prg, the modelling of the circuit meshes in a graph structure has been done @chen_graph_2020. In @fig_graph_representation, one can see the graph representation of a single photonic gate and a junction in a hexagonal photonic mesh, and in @fig_graph_representation_mesh, one can see a set of gates in a mesh. In their work, _Xiangfeng Chen et al._ show that by incorporating relevant metrics in the mesh edges, they can achieve efficient routing in a photonic mesh. This is a very promising result and can be used as the basis for future research. Indeed, research is already ongoing at Ghent University to further this routing. However, they are not yet at the point of incorporating more complex photonic components inside of the mesh @kerchove_adapting_2022. Other work has implemented the automatic realisation of circuits on photonic meshes, which is closer to what is needed for synthesis, but it is still incomplete @gao_automatic_2022.
...
#ufigure(
  ...
  caption: [
    Graph representation of a single photonic gate (a) and of a complete junction in a photonic mesh (b). Based on the work of _Xiangfeng Chen et al._ @chen_graph_2020. (b) is composed of three unit cells shown in (a), showing the direction that light is travelling in and all of the possible connections.
  ],
  kind: image,
  table(
    columns: 2,
...
#udefinition(footer: [ Adapted from @z3prover. ])[
...
A prover can express the mathematical relations between constraints; this means that a prover like _Z3_ can be used to check whether two constraints are compatible. This would be used as part of the compilation and evaluation processes.
...
#ufigure(
  ...
  caption: [
    Simulation results of the marshalling layer example, showing the output of the modulator, and the output directly from the splitter. The output of the modulator is the same as the output of the splitter, but with the PRBS sequence modulated onto it.
  ],
  image(
    "../assets/simu_marshalling_ex.svg",
    width: 100%,
    alt: ""
  )
 ) <fig_marshalling_sim>

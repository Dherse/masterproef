...
#ufigure(caption: [ Functional requirements for intent translation ], kind: table)[
  #tablex(
...
#udefinition(footer: [ Adapted from @huang_virtualization_2018. ])[
...
==== User @hal
The user @hal is a higher-level part of the @hal built from the user's design. It encapsulates the tunable values, reconfiguration states and detectors that are defined within the design and allows the user to change these values, reconfigure the device and readout detectors. All the while using the names the user-defined for these different values. This allows the user to interact with the device in a way consistent with their design, making it easier to understand and use. This should improve productivity and reduce the risk of error in the hardware-software codesign interface.

==== User @hal template
The user @hal needs to be generated from the user's design; however, there may be elements of this @hal that are platform specific and, therefore must be generated instead by the device support package. This is expected to be done by allowing the device support package to generate part of the user @hal through a template or custom code. This allows the device support package to provide platform-specific features to the user @hal or to optimise common implementations for the platform, further improving the quality and usability of the generated interface.

== Intrinsic operations <sec_intrinsic_operations>

From the physical properties and features of a photonic processor, as discussed in @sec_programmable_photonics, one can extract a set of intrinsic operations that the processor must support. These operations in themselves are not required to be on the chip, but the support packages of the chip must be able to understand them and produce errors when they are not implemented. A complete list with a description can be found in @tab_intrinsic_operations. In this section, the intrinsic operators will be discussed in more detail.

==== Filter
One of the core operations that almost all photonic circuits perform is filtering. It is a block that alters the amplitude of an input signal in a predictable manner based on its spectral content. Due to the prevalence of filters in photonic circuits, coupled with their special constraint -- see below -- they benefit from being an intrinsic operation for a photonic processor. During compilation and before place-and-route, the filter will be synthesised based on its arguments in order to produce a filter of the desired frequency response. This synthesised filter will therefore be optimised for the hardware platform.

There are many different types of filters, the most common ones that can easily be implemented on a mesh -- are @mzi[s], ring resonators, and lattice filters. Additionally, compound filters that combine multiple types of filters or more than one filter can be created; such filters can have improved response or behave like bandpass filters. Therefore, the compiler's task is to choose the base filter based on the specification and performance criteria that the user has set. For example, the user might prioritise optimising for mesh usage rather than finesse or might optimise for flatness of the phase response rather than the mesh usage, etc.

==== Gain and loss
All waveguides within a device will cause power loss in the optical signals. However, this loss may not be sufficient if the user is working with a high-power signal. Therefore some devices might include special loss elements whose loss is tunable or at least known. Besides, following the same principle, some users may want to compensate for this loss by using gain sections or amplify incoming signals. Optical gain is difficult to obtain on silicon platforms, just like sources, but it is possible to obtain gain through the use of rare-earth doped waveguides or other techniques such as micro-transfer printing. Therefore, the compiler must be able to synthesise gain and loss sections based on the user's specifications. However, if the device does not support gain or loss, the compiler should produce an appropriate error for the user.

==== Modulator and detectors
Two key applications of photonic processors are telecommunication and processing @rf signals. Therefore, it stands to reason that modulators and detectors are key components that are expected to be present in most photonic processors. Additionally, based on the device, there may be an optimal type of modulator for either type -- phase modulation or amplitude modulation -- and the compiler may choose an appropriate implementation of the modulator. Additionally, the same is true for detectors, although they would generally only be used for amplitude demodulation, with phase coherent demodulation being the responsibility of the user.
...
#ufigure(
  ...
  #tablex(
    ...
    smallcaps[*Splitter*],
    align(left)[
      A splitter that splits an optical signal into multiple lower-power optical signals.
    ],
    align(left)[
      - #required_sml#lightbulb#h(0.1em) Input signal
    ...
    smallcaps[*Combiner*],
    align(left)[
      A combiner that combines multiple optical signals into a single optical signal. At the same time, maximising the total output power.
    ],
    align(left)[
    ...
    align(left)[
      A combiner that interferes multiple optical signals into a single optical signal. Does not perform any power optimisation.
    ],
    align(left)[
      - #required_sml#lightbulb#h(0.1em) Input signals
      - #required_sml#lightbulb#h(0.1em) Output signal
...
#ufigure(
  ...
  caption: [
    Hierarchy of constraints, showing that all constraints are simulation constraints, within that are meta constraints within which are synthesisable constraints.
  ],
  kind: image,
  image(
...
#ufigure(
  ...
  #tablex(
    ...
    align(left)[
      Wavelength constraints are used to specify the wavelength content of a signal. This is used for the optimisation of filters and other wavelength-dependent components.
    ],
    ...
    smallcaps[Phase],
    align(left)[
      Phase constraints are used to specify the differential phase of a signal. This can be used to ensure that phase-sensitive circuits are able to work as intended.
    ],

    rowspanx(2)[#rotate(-90deg)[#box(width: 200pt)[#smallcaps[Simulation]]]],
    smallcaps[Noise],
    align(left)[
      Noise constraints are used to add noise to a signal. This can be used to simulate noise sources and to simulate the impact of noise on a device.
    ],
...
==== Constraint solver
The solver is the tool that the compiler uses to summarise and check constraints. It will summarise the constraints on each signal such that it can easily be simulated, and it will verify that constraints are compatible. Additionally, in cases where the constraints depend on tunable values -- i.e. values that can be changed at runtime -- the solver can use a prover and the constraints on the tunable value to determine whether the constraints are compatible. This is done by using a prover such as _Z3_ @z3. However, this is a very computationally expensive process and must therefore, only be performed when necessary. The compiler will only use the prover when the constraints depend on tunable values. Therefore, one can see the constraint solver as a tool composed of two subsystems, the first one, computing and verifying constraints based on known data, it is simpler and faster, and the second one, computing and verifying constraints based on tunable values, it is more complex, relying instead on a prover. 

==== Limitations of constraints
There are, however limitations of the constraint system, most notably that, using the aforementioned constraint solver, constraints are limited to exclusively feedforward system. This is due to constraints being calculated one after the other, not allowing cyclic dependencies. And as discussed in @feedforward_approx, one can represent any recirculating circuit as a feedforward system. However, there is one necessary condition for this to hold: it _must_ be at a higher level of abstraction. When building the higher-level abstractions, this axiom cannot be assumed to be true, as the user is writing them at a lower level of abstraction. Therefore, the constraint system is limited in such cases, and the system must provide an "escape hatch" which allows the user to manually specify constraints at the edges of their abstractions, such that the compiler can use them outside of the abstraction while pausing constraint computation inside. When using this feature, the user can now express recirculating circuits easily by using the escape hatch to specify constraints on insufficient feedforward signals.
...
Tunable values are values that can be interacted with, by the user, at runtime. They can be any non-signal value in the user's program, typically numerical values, that the user defines as being tunable values. These values can be seen as tuning knobs that the user can access at runtime to change their circuit's behaviour and implement their custom feedback loops. Tunable values can impact several parts of the design at once. For example, a single value may determine the centre frequency of operation of a bank of filters, all of them being tuned at once. This makes tunable values especially powerful as their impact can be propagated through the entire design.
...
Additionally, the user should be able to name and access their tunable values by name within their code. This further improves the usability of the system, as it removes the need to maintain complex, error-prone tables of registers and their corresponding values. Instead, the user can address their tunable value naturally through its name, and the @hal can take care of the rest: translating these names and values into an appropriate set of registers and values.
...
Furthermore, the use of constraints on values, such as explained in @sec_constraints, can be used by the compiler to further detect the need for reconfigurability automatically without additional user input. It can also be used to validate that when a tunable value is changed in the user's software, it meets its requirements, ensuring that the user cannot change the value to an invalid one, where the device might then operate in an undefined state.

#udefinition[
  *Reconfigurability* is the ability to change the topology of the device at runtime and, therefore its behaviour.
]

Additionally, one of the most important functional requirements is the reconfigurability. Its goal is to allow the user to reconfigure the mesh -- or only parts of it -- while the device is running. This can be achieved in several ways, but the most natural way is to use branches within the code to determine the boundaries between reconfigurability regions. Then, through the use of tunable values, these regions can be automatically selected based on their value.
...
Neither of those two tasks is trivial, they can be seen as similar to the halting problem, which is undecidable, and therefore, it is desirable to let the user specify some of the state reduction manually, letting the user take care of parts of the more complex cases. One can draw a parallel between this and the use of the escape-hatch for constraints, as it is a similar concept, where the user can specify constraints manually at the edge of abstraction, while here, the user can specify how to reduce the number of states manually. However, limiting the maximum number of iterations, the recursion depth, or both can make the halting problem decidable. However, despite being decidable, it still incurs a high computational cost. Therefore, one can expect that state reduction will also be computationally expensive.

In @fig_reconfigurability, one may see what reconfigurability might look like on a fictitious device, where based on an input variable, a simple boolean in this case, the device will use either of two meshes. Each state #link(<fig_reconfigurability>)[(a)] and #link(<fig_reconfigurability>)[(b)] represents a different mesh that implements a different filter. In this example, the user would have created a tunable boolean that they can set at runtime, and based on its value, the appropriate mesh will be selected.

#ufigure(
  kind: image,
  outline: [ Example of reconfigurability on a fictitious device.],
  caption: [
    Example of reconfigurability on a fictitious device. Each state (a) and (b) represent a different filter. The second (b) filter has a longer ring and a higher @fsr than the first one (a). Squares of different colours represent photonic gates in different states: blue represents through gates, green represents cross gates, and yellow represents partial gates. The grey triangles represent optical ports.
  ],
  table(
    columns: 2,
    stroke: none,
    image("../assets/drawio/reconf_ex_a.svg", alt: "Shows a photonic processor's mesh configured with a simple ring resonator filter.", width: 200pt),
    image("../assets/drawio/reconf_ex_b.svg", alt: "Shows a photonic processor's mesh configured with a simple longer ring resonator filter.", width: 200pt),
    [(a)], [(b)],
  )
) <fig_reconfigurability>

#uconclusion[
    Reconfigurability allows the user to create modular designs, where, at runtime, the user can select a different state to fit their needs. Reconfigurability is achieved through the branching of the code. The user can specify tunable values that are used to select the appropriate branch. The number of states is exponential but can be decreased using the constraint solver to remove unnecessary branches by finding independent mesh subsets and letting the user manually specify some of the state reduction.
]
...
Photonic circuits can be susceptible to manufacturing variations and temperature variations. Therefore, each device must come with mitigation techniques that can aid in making the device behave as ideally as possible. This is expected to generally be done using calibration curves or calibration @lut[s]. And by using feedback loops to ensure that a component behaves as expected. For example, a power combiner might maximise power output using a feedback loop on a phase shifter to create constructive interference.
...
tablex(
    ...
    align(left)[
      High-speed modulators are used to modulate the light; they can operate either in a phase modulation scheme or be used with a @mzi to perform amplitude modulation.
    ],

    smallcaps[*Laser source*],
    align(left)[
      Laser sources are used to generate light at a given wavelength directly inside the device. Due to the devices being made in silicon, there are none on existing prototypes. However, they may be added using epitaxial growth or micro-transfer printing.
    ],

    smallcaps[*Gain section*],
    align(left)[
      Gain sections are used to amplify the light, generally made of a semiconductor optical amplifier or an erbium-doped waveguide section. As with laser sources, there are currently no gain sections on prototypes.
    ],

    smallcaps[*Optical port*],
    align(left)[
      These ports at the edge of the device can be used to couple light in and out of the device.
    ],

    smallcaps[*Switch*],
    align(left)[
      Switches can be either implemented using the mesh and couplers, using a power splitter with its coupling coefficient controlled by a tunable value, or built into the device as dedicated hardware. There are no dedicated hardware switches, but they may be added in future devices.
    ],
  )
 ) <tab_resources>
